...
namespace VRQuestionnaireToolkit
{
    public class GenerateQuestionnaire : MonoBehaviour
    {
        ...
        private int _numQuestionnaire = 0;
        [SerializeField] GameObject questionnaireObject;

        public void SetNumQuestionnaire(int num)
        {
            Debug.Log("SetNumQuestionnaire: " + num);
            _numQuestionnaire = num;
        }

        private void FireEvent()
        {
            Questionnaires[_numQuestionnaire].SetActive(false);
            _numQuestionnaire += 1;

            bool nextQuestionnaire = Questionnaires.Count > _numQuestionnaire;
        ...
        void Start()
        {
            ...
            for (int i = 0; i < Questionnaires.Count; i++)
            {
                ...
                Questionnaires[i].transform.localScale = new Vector3(1, 1, 1);
            }

            //Questionnaires[numQuestionnaire].SetActive(true);
        }


        public void ShowQuestionnaire()
        {
            questionnaireObject.GetComponent<Canvas>().renderMode = RenderMode.ScreenSpaceOverlay;

            for (int i = 0; i < Questionnaires.Count; i++)
                Questionnaires[i].SetActive(false);

            Questionnaires[_numQuestionnaire].SetActive(true);
        }
