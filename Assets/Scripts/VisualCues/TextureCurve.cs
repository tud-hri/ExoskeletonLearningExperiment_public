...
[CreateAssetMenu(fileName = "TextureCurve", menuName = "ScriptableObjects/TextureCurve", order = 1)]
public class TextureCurve : ScriptableObject
{
    ...
    public void SetRedKeyKeyTime(int keyNum, float time)
    {
        AnimationCurve curve = new AnimationCurve();
        Keyframe[] keys = (Keyframe[])_red.keys.Clone();
        keys[keyNum].time = time;
        curve.keys = keys;
        keys[keyNum].inTangent = 

        _red.MoveKey(keyNum, new Keyframe(time, _red.keys[keyNum].value));
    }

    public float EvaluateRedCurve(float t) => _red.Evaluate(t);
